...
class CartController extends Controller
{
    ...
    {
       /*  $basket = $this -> basket ; */
       $products=Product::where('inCard',1)->get();
    ...
    public function postAdd(Request $request)
    {
        $name =$request -> product_name ;
         ...
         $data=Product::where('incard',1)-> count();

        try {
             if( $product->inCard==Null|| $product->inCard==0)
             {
                $product->update(['inCard' => 1]);

                return response() -> json(['data'=>$data,'status' => true , 'card' => true]);
             }
             else{
                return response() -> json(['data'=>$data,'status' => true , 'card' => false]);
             }
    ...
    public function postUpdate($name, Request $request)
    {
        ...
        return response() -> json(['status' => true , 'card' => true]);
    }
